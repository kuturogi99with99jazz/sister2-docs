...
一覧上の各Workに「コピー」アクションを提供し、コピーは既存Workを基に新規登録フォームを起動できること。
コピー起動時は、参照元Workの主要項目を初期値としてセットし、ユーザーが必要に応じて編集できること。
コピーしたWorkは新規Workとして扱い、work_id、created_by、作成日時は新規発番とすること。
コピー時はステータス、実績日付、コメント、関連Work、添付ファイルを初期値から除外し、それ以外の項目は引き継ぐこと。
...
- [Decision] Work完了判定条件: ステータスが Completed になった時点で完了扱い
...
対象の粒度はシステムとし、共通業務は対象テーブルではなくタグで表現すること。

#### 4.3.1 追加検討事項

- [Decision] 対象の粒度: システム
- [Decision] 共通業務の表現方法: タグ
...
- [Decision] 稼働時間は操作ログの開始日時・終了日時の差分で算出する
- [Decision] 開始日時の入力時点と終了日時の入力時点をログとして記録し、その差分を稼働時間とする
- [Decision] 後から手入力で開始/終了日時を編集でき、編集後も差分を再計算する
- [Open Question] 並行作業の扱い（重複除外/重み付け）
- [Decision] 重み付け配賦方式を採用する（例: 2タスク同時なら各タスクに0.5ずつ配賦、重要度や進捗比率に応じて0.7/0.3など可変）
- [Open Question] 重みの算定ルール（重要度/進捗比率の定義・入力方法・優先順位）をどう定義するか
...
本文はMarkdownで記述し、表示用のHTMLを保持できること。  
初期対応の記法は見出し/リスト/リンク/画像/コードブロック/インラインコード/太字/斜体/引用/水平線を対象とする。  
下書きは作成者のみ編集でき、公開後は権限に応じて編集/非公開にできること。  
記事に添付資料（PDF/画像/スプレッドシート等）を紐付けできること。  
記事本文に埋め込む画像はS3の社内限定公開バケットに保存し、1ファイル1MB上限とする。  
社内限定公開バケットは、社内IP/VPNからの読み取りのみ許可するバケットポリシーを前提とする。  
...
画面最上部（ヘッダの上）にお知らせバナーを表示できること。  
バナーはタイトル/本文/リンク/優先度/表示開始日時/表示終了日時を持つこと。  
バナーは組織単位/全体/対象（例: 管理者のみ）で出し分けできること。  
ユーザーはバナーを閉じることができ、同一内容は再表示されないこと。  
バナーは共通コンポーネントとしてヘッダ上に常備し、条件に応じて表示/非表示を切り替えること。  

- [Assumption] お知らせの掲載/管理は管理者のみが行える
- [Risk] 乱用によるノイズ化を防ぐため、掲載期間と優先度の運用ルールが必要
...
ナレッジ本文内の画像は社内限定公開バケットに保存し、署名付きURLの対象外とする。
...
- [Decision] 稼働時間は操作ログの開始/終了入力の差分で算出し、後からの手入力修正にも対応する
...
| レイヤ | 概要 | 例 |
|------|------|------|
| システム権限（Role） | 役割ごとの機能範囲を定義 | ゲスト / パートナー / 担当 / リーダー / マネージャー / ディレクター / 運用者 / 管理者 |
| データ権限（Scope） | どのWorkが見えるか | 自分のWork / 関与システムのWork / 全体 |
| 操作権限（Action） | 何ができるか | 閲覧 / 作成 / 更新 / 完了 / 管理 |

「見える」と「操作できる」を分離し、誤操作と過剰な権限付与を防止すること。
Roleは「ゲスト / パートナー / 担当 / リーダー / マネージャー / ディレクター / 運用者 / 管理者」に限定すること。
...
管理業務の操作は監査ログに記録し、編集権限は運用者/管理者で区分すること。  
管理UIは「運用者向け（限定権限）」と「管理者向け（フル権限）」を分離すること。  
運用者Roleを新設し、管理者Roleとは権限範囲を明確に分離すること。  
メンテ対象は「マスタ系＋運用で必要な一部のみ」とし、テーブル値編集は管理UI/API経由を前提とすること。  
設定/定義/運用系は「マスタメンテ画面の一部」として同一ナビで管理すること。  
設定変更は履歴を保持し、差分表示ができること。

#### 4.13.1 マスタ管理機能

以下のマスタを必須とすること：ユーザー、会社、拠点、部門、ロール、リソース、パーミッション、タグ、カテゴリ、Work分類、Work対象。  
設定/定義/運用系のメンテ対象に、system_settings、company_settings、internal_tools、tool_definitions、report_templates、screen_feedback_templates、announcements を含めること。  
...
- [Open Question] 運用者が編集可能なマスタ/運用項目の範囲（masterTypeの一覧）
