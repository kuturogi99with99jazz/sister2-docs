...
| ストレージ | S3 | ファイル・添付・一時共有データ、ナレッジ画像（社内限定公開・1MB上限） |
...
管理UIは「運用者向け（限定権限）」と「管理者向け（フル権限）」を分離し、データメンテは管理UI/API経由を前提とする。  
メンテ対象は「マスタ系＋運用で必要な一部のみ」とし、トランザクション系の直接編集は行わない。

ナレッジ本文はMarkdownで保存し、表示用HTMLを保持する。初期対応の記法は見出し/リスト/リンク/画像/コードブロック/インラインコード/太字/斜体/引用/水平線とする。  
ナレッジ画像用のS3バケットは社内限定公開とし、社内IP/VPNからの読み取りのみ許可する。

社内限定公開バケットのポリシー例（IP制限）:
```json
{
  "Version": "2012-10-17",
  "Statement": [
    {
      "Sid": "AllowReadFromCorpIp",
      "Effect": "Allow",
      "Principal": "*",
      "Action": "s3:GetObject",
      "Resource": "arn:aws:s3:::sister-knowledge-images/*",
      "Condition": {
        "IpAddress": {
          "aws:SourceIp": [
            "203.0.113.0/24",
...
Roleは「ゲスト / パートナー / 担当 / リーダー / マネージャー / ディレクター / 運用者 / 管理者」に限定し、業務の違いはScopeで表現する。

| レイヤ | 目的 | 例 |
|------|------|------|
| Role | システム機能の上限 | ゲスト / パートナー / 担当 / リーダー / マネージャー / ディレクター / 運用者 / 管理者 |
...
| file_upload | S3署名URL発行 | API Gateway | 添付は署名付きURL、ナレッジ画像は公開バケット |
