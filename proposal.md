...
主な利用者はSIの担当者とし、管理者は参照と承認に必要な範囲で利用する想定とする。
...
| 大分類 | 機能 | 説明 | 課題 |
|--------|------|------|------|
| プロジェクト管理 | 一覧・登録・削除 | 進捗・担当者・日付を管理 | 業務の性質/対象の表現が弱い |
...
- 通知・ファイル管理の非同期化・自動化
- ナレッジ本文はMarkdownで保存し、表示はレンダリングHTMLとする（見出し/リスト/リンク/画像/コードブロック/インラインコード/太字/斜体/引用/水平線を初期対応）
- ナレッジ画像はS3の社内限定公開バケットに保存し、1ファイル1MB上限・社内IP/VPN限定アクセスとする
...
- **S3署名URL（添付）+ 社内限定公開バケット（ナレッジ画像）+ SES/SNS通知**によるファイル管理・通知機能  
