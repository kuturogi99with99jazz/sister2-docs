...
## 1. 背景
既存の「Sister」システムは、2023年にExcelで行っていたプロジェクト進捗管理をWebシステム化したものです。
...
| 大分類 | 機能 | 説明 | 課題 |
|--------|------|------|------|
| プロジェクト管理 | 一覧・登録・削除 | 進捗・担当者・日付を管理 | UIが雑、情報不足 |
| タスク管理 | CRUD / 状態変更 | ステータス・期日・担当者を追跡 | 拡張しづらい |
| ナレッジ管理 | ブログ／ドラフト管理 | 記事投稿・タグ付け | 実装途中で停止 |
| 通知機能 | SES/SNS送信 | タスク登録通知 | 通知が見づらい |
| ファイル管理 | S3アップロード | 署名付きURL発行 | 実装したが本格的な活用なし |
...
- プロジェクト・タスク・チャット・社内ツールに蓄積される情報を対象に、
  要約・補完・提案を行う
- 定義駆動型の社内ツールに対し、入力補助・内容チェック・自動要約を提供
- 人が行う判断・承認を置き換えるのではなく、
  作業負荷の軽減および意思決定支援を目的とする
- 初期段階ではバッチ処理・オンデマンド実行を中心とし、
  利用状況に応じてリアルタイム連携を検討する
...
- **AI連携による補助**で作業効率化（タスク要約・レポート生成）※  
- **低コスト運用**（月額数百円レベルを目指す）  

>※ AI連携による業務工数削減  
>週次・月次レポートをAIが自動生成し、報告作成時間を約50〜70%削減  
>タスク履歴・コメントを自動要約し、進捗確認・引き継ぎ工数を最小化
...
|-----------|------|------|
| 1. 要件定義 | 2週間 | 機能・構成の整理 |
| 2. PoC開発 | 3週間 | Lambda・Vercel・Neonの連携確認 |
| 3. 実装 | 4〜6週間 | フロント・バックエンド構築 |
| 4. テスト・検証 | 2週間 | 結合／負荷／運用テスト |
| 5. リリース | - | 本番運用開始 |
